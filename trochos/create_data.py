...
class DataCreator:
    def __init__(self, tickers, months):
    ...
    def get_and_write(self):
        ...
        # Writing to Parquet
        print("writing data!")
        combined_df.to_parquet('ml/example/historical_data.parquet', compression='snappy')
            
    def get_df_for_ticker(self, ticker):
            history = tradier.get_historical_data(ticker, self.months)
            df_per_ticker, err = get_df_from_history(history)
            if err != None:
                return None, err.wrap(f"Could not construct pandas.DataFrame for {ticker}")
            return (ticker, df_per_ticker), None


def get_df_from_history(history):
...
if __name__ == "__main__":
    ...
    creator = DataCreator(tickers, args.months)
